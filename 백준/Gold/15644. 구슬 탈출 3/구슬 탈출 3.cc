...
pair<int, string> bfs(int start_rx, int start_ry, int start_bx, int start_by){
    
    // Red 좌표, Blue좌표, 거리, 방향기록
    ...
    while (!q.empty()){
        auto [rx, ry, bx, by, dist, way] = q.front();
        ...
        for (int i=0; i<4; i++){
            ...
            if (nrx==-1) return {dist+1, way+direct[i]};

            if (visited[nrx][nry][nbx][nby]) continue;
            visited[nrx][nry][nbx][nby] = true;
            q.push({nrx, nry, nbx, nby, dist + 1, way+direct[i]});
        }
    }
    return {-1, ""};  // 도달 못했을 경우
}
...
int main(){
    ...
    pair<int, string> ans = bfs(rx, ry, bx, by);
    cout << ans.first << '\n';
    cout << ans.second;
