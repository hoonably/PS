...
const int SZ = 410, SRC = 401, SINK = 402, bias = 200;
    ...
    void printCut(int n, int S){
        ...
        while(!q.empty()){
            ...
            for(auto nxt : graph[now]){
                ...
                // 잔여 유량이 있다면
                if(nxt.cap > 0){
                    q.push(nxt.to); chk[nxt.to] = true;
                }
            }
        }

        for(int i=1; i<=n; i++){
            if(chk[i] && !chk[i+bias]) cout << i << " ";
        }
    }
} nf;
...
int main(){
    ...
    for (int i=1; i<=n; i++){
        int cost;
        cin >> cost;
        nf.addEdge(i, i+bias, cost);
    }
    
    for(int i=0; i<m; i++){
        int a, b;
        cin >> a >> b;
        nf.addEdge(a+bias, b, INF);
        nf.addEdge(b+bias, a, INF);
    }
