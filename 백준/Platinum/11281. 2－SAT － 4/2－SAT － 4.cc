...
// x1, x2 ... 값을 출력하기 위한 배열
bool visited[2*MAX_N], ans[2*MAX_N];
...
int main(){
    ...
    // 변수들의 true/false 값을 저장하여 출력
    for (vector<int> scc : SCC) {
        for (int num : scc) {
            if (visited[num%2==1 ? num^1 : num]) continue;

            // num의 값이 홀수(Not)라면 그에 해당하는 순서의 변수를 false
            if (num%2==1) {
                ans[num^1] = false;
                visited[num^1] = true;
            }
            // num의 값이 짝수(OK)라면 그에 해당하는 순서의 변수를 true로 
            else {
                ans[num] = true;
                visited[num] = true;
            }
